...
>>> let p = mk [1,3,2]; q = mk [3,1,2]; r = mk [1,5,3,6,2,4]) in shuffleOf2 p q r
True
-}
...
>>> PP.Perm.mk [1,3,2] == PP.Perm.mk [1,5,3] && PP.Perm.mk [3,1,2] == PP.Perm.mk [6,2,4] -- check first solution
True
>>> PP.Perm.mk [1,3,2] == PP.Perm.mk [1,6,2] && PP.Perm.mk [3,1,2] == PP.Perm.mk [5,3,4] -- check second solution
True
-}
shuffleOf2' :: PP.Perm.Perm -> PP.Perm.Perm -> PP.Perm.Perm -> [([PP.Geometry.Point.Point], [PP.Geometry.Point.Point])]
shuffleOf2' p q = L.filter (T.uncurry (==) . (PP.Perm.mk A.*** PP.Perm.mk)) . PP.Perm.partitions np nq
  where
    np = PP.Perm.len p
    nq = PP.Perm.len q
...
>>> let p = PP.Perm.mk [1,2]; q = PP.Perm.mk [2,1] in shuffle2 p q
...
>>> let p = PP.Perm.mk [1,2]; q = PP.Perm.mk [2,1]; r = PP.Perm.mk [2,1] in shuffle3 p q
-}
...
>>> let p = PP.Perm.mk [1,2]; q = PP.Perm.mk [1,2]; r = PP.Perm.mk [2,1]; r = PP.Perm.mk [2,1] in shuffle4 p q r s
-}
...
[[1,3,4,2],[1,3,4,2],[1,3,2,4],[3,1,4,2],[3,1,2,4],[3,1,2,4]]
>>> shuffle [mk [1,2], mk [2,1], mk [1,3,2]]
