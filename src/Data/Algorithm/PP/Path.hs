...
endHeightPaths h n
  | h < 0  = complement <$> endHeightPaths (-h) n
  | h > n  = []
  | h == n = [upSteps h]
  | otherwise = ps ++ ps'
    where
      ps  = (downSteps 1 <>) <$> endHeightPaths (h+1) (n-1)
      ps' = (upSteps 1 <>)   <$> endHeightPaths (h-1) (n-1)
...
(4,[(((()])
-}
exactStartUpStepPaths :: Int -> Int -> [Path]
exactStartUpStepPaths k n = fmap (p <>) $ paths (n-k)
  where
    p = upSteps k <> downSteps 1
...
endDownStepPaths k = fmap rev . startUpStepPaths k
...
(4,[())))])
-}
exactEndDownStepPaths :: Int -> Int -> [Path]
exactEndDownStepPaths k = fmap rev . exactStartUpStepPaths k
...
(4,0,[((((])
-}
startUpStepEndDownStepPaths :: Int -> Int -> Int -> [Path]
startUpStepEndDownStepPaths k l n = [upSteps k <> p <> downSteps l | p <- paths (n-k-l)]
...
(4,0,[])
-}
exactStartUpStepEndDownStepPaths :: Int -> Int -> Int -> [Path]
exactStartUpStepEndDownStepPaths k l n = [upSteps k <> downSteps 1 <> p <> upSteps 1 <> downSteps l | p <- paths (n-k-l-2)]
